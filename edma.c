...
uint32_t* edma_get_Data() {
    return &FRAME_TO_TRANSFER[readBuffer][0];
}
...
uint8_t edma_init_PaRAM() {
    ...
    (EDMA_PaRAM)->dst = (uint32_t)(DATA_MEMORY_BASE_ADDRESS + (uint32_t)(&FRAME_TO_TRANSFER[writeBuffer][0]));
    ...
    (EDMA_PaRAM + 1)->dst = (uint32_t)(DATA_MEMORY_BASE_ADDRESS + (uint32_t)(&FRAME_TO_TRANSFER[writeBuffer][0]));
